...
static void
slab_cache_push_back(struct slab_cache *cache,
	struct slab *slab) {
...
static void
slab_cache_remove(struct slab_cache *cache,
	struct slab *slab) {
...
int
slab_cache_init(struct slab_cache *cache,
...
void
slab_cache_dealloc(struct slab_cache *cache,
	void *ptr) {
	size_t const pagesize = GET_PAGESIZE();
